...
type Salesman = {
    ...
    shared_sale_commission_percent: string;
...
export default function Salesmen({
    ...
    const form = useForm({
        ...
        shared_sale_commission_percent: '0',
    ...
    const selectSalesman = (salesman: Salesman) => {
        ...
        form.setData({
            ...
            shared_sale_commission_percent:
                salesman.shared_sale_commission_percent ?? '0',
            permissions: {
                ...blankPermissions,
                ...Object.fromEntries(
    ...
    const openReport = async () => {
        ...
        try {
            const response = await fetch(
                `/management/salesmen/${selected.salesman_id}/report`,
                {
    ...
    const openCommissionReport = async () => {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error('Unable to load commission report');
            setReport((await response.json()) as SalesmanReport);
        } finally {
            setReportLoading(false);
        }
    ...
    const downloadCommissionPdf = () => {
        ...
        const lines = [
            ...
            ...report.commission.rows.flatMap((row) => [
                ...
                `Sale ${reportMoney.format(row.total_sale)} | Expenses ${reportMoney.format(row.expenses)} | Lead cost ${reportMoney.format(row.initial_cut)} | Commission base ${reportMoney.format(row.commission_base)} | Due ${reportMoney.format(row.commission_due)} | Paid ${reportMoney.format(row.commission_paid)} | Owed ${reportMoney.format(row.commission_balance)}`,
                '',
            ]),
        ];
        ...
        pages.forEach((page, index) => {
            ...
            page.forEach((line, lineIndex) => {
                content += `1 0 0 1 38 ${750 - lineIndex * 17} Tm (${escape(line)}) Tj\n`;
            });
            content += 'ET\n';
            objects[pageId] =
        ...
        const url = URL.createObjectURL(
            new Blob([new TextEncoder().encode(pdf)], {
                type: 'application/pdf',
            }),
        );
        const link = document.createElement('a');
        link.href = url;
        link.download = `${selected.salesman_name.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}-commission-report.pdf`;
        link.click();
            ...
            <main className="agents-page salesmen-page">
                <header className="agents-header directory-heading-with-total">
                    <div className="directory-heading-copy">
                        ...
                        <h1>Salesmen</h1>
                        <p>
                            Create and maintain salesman records in Weiss CRM.
                        </p>
                    </div>
                    ...
                    >
                        <div className="agents-directory-heading">
                            <div className="directory-heading-title-row">
                                ...
                                <span className="directory-inline-count">
                            ...
                            <section className="salesman-commission-card">
                                <header>
                                    ...
                                    <div className="salesman-commission-totals">
                                        ...
                                        <span>
                                            <small>Calculated cut</small>
                                            <strong>
                                                {reportMoney.format(
                                                    selected?.completed_cut_total ??
                                                        0,
                                                )}
                                            </strong>
                                        </span>
                                ...
                                <div className="salesman-commission-fields">
                                    {(
                                        [
                                            [
                                                'initial_sale_cut_percent',
                                                'Lead cost',
                                            ],
                                            [
                                                'change_order_cut_percent',
                                                'Change order lead cost',
                                            ],
                                            [
                                                'sale_commission_percent',
                                                'Additional sale commission',
                                            ],
                                            [
                                                'shared_sale_commission_percent',
                                                'Shared-project commission',
                                            ],
                                        ] as const
                                    ).map(([field, label]) => (
                                        <label key={field}>
                            ...
                            <div className="agents-form-actions">
                                {selected && (
                                    <>
                                        <button
                                            type="button"
                                            className="agents-report-button"
                                            onClick={openReport}
                                        >
                                            <BarChart3 /> Salesman report
                                        </button>
                                        <button
                                            type="button"
                                            className="agents-report-button"
                                            onClick={openCommissionReport}
                                        >
                                            <BarChart3 /> Accounting report
                                        </button>
                                        <button
                                            type="button"
                ...
                <Dialog open={reportOpen} onOpenChange={setReportOpen}>
                    <DialogContent className="salesman-report-modal">
                        <DialogHeader>
                            <DialogTitle>
                                {selected?.salesman_name} — Salesman report
                            </DialogTitle>
                            <DialogDescription>
                                Leads and projects assigned as primary or second
                                salesman.
                            </DialogDescription>
                        </DialogHeader>
                        {reportLoading ? (
                            <div className="agent-report-loading">
                                Loading report…
                            </div>
                        ) : report ? (
                            <>
                                <div className="salesman-report-summary">
                                    ...
                                    <span>
                                        <small>Sold</small>
                                        <strong>{report.summary.sold}</strong>
                                    </span>
                                    <span>
                                    ...
                                    <span>
                                        ...
                                        <strong>
                                            {reportDate(
                                                report.summary.last_sale,
                                            )}
                                        </strong>
                                    </span>
                                </div>
                                <div className="salesman-report-table-wrap">
                                    <table>
                                        <thead>
                                            <tr>
                                                <th>Origin</th>
                                            ...
                                            {report.rows.map((row) => (
                                                <tr key={row.id}>
                                                    ...
                                                    <td>
                                                        {row.project_id ? (
                                                            <a
                                                                href={`/management/projects?project=${row.project_id}&tab=INV`}
                                                            >
                                                                {
                                                                    row.project_number
                                                                }
                ...
                <Dialog open={commissionOpen} onOpenChange={setCommissionOpen}>
                    <DialogContent className="salesman-report-modal salesman-accounting-report-modal">
                        <DialogHeader>
                            <DialogTitle>
                                {selected?.salesman_name} — Accounting &amp;
                                commission report
                            </DialogTitle>
                            <DialogDescription>
                        ...
                        {reportLoading ? (
                            ...
                            <>
                                <div className="salesman-report-summary">
                                    ...
                                    <span>
                                        ...
                                        <strong>
                                            {reportMoney.format(
                                                report.commission.summary.sales,
                                            )}
                                        </strong>
                                    </span>
                                    <span>
                                ...
                                <div className="salesman-report-rates">
                                    <span>
                                        Lead cost{' '}
                                        <b>
                                            {
                                                report.commission.rates
                                                    .initial_sale
                                            }
                                            %
                                        </b>
                                ...
                                <div className="salesman-report-table-wrap">
                                    <table className="salesman-accounting-table">
                                        <thead>
                                            <tr>
                                                <th>Project</th>
                                                <th>Customer</th>
                                        ...
                                        <tbody>
                                            {report.commission.rows.map(
                                                (row) => (
                                                    <tr key={row.project_id}>
                                                        <td>
                                                            <a
                                                                href={`/management/projects?project=${row.project_id}&tab=DTL`}
                                                            >
                                                                {
                                                                    row.project_number
                                                                }
                            ...
                            <div className="agent-report-loading">
                                The accounting report could not be loaded.
                            </div>
                        )}
                    </DialogContent>
                </Dialog>
